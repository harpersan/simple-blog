...
{{-- {{ dd(App\Post::blogPost(0,1)) }} --}}
<div class="bg-gray-200 font-sans leading-normal tracking-normal">
<div id="app" class="w-full m-0 p-0 bg-cover bg-bottom"
...
<div class="container px-4 md:px-0 max-w-6xl mx-auto -mt-32">
    ...
    <div class="mx-0 sm:mx-6">
        ...
        <div class="bg-gray-200 w-full text-xl md:text-2xl text-gray-800 leading-normal rounded-t">
            ...
            @foreach (App\Post::blogPost(0,1) as $blogpost)
            <!--Lead Card-->
            <div class="flex h-full bg-white rounded overflow-hidden shadow-lg">
                <a href="/view/{{$blogpost->id}}" class="flex flex-wrap no-underline hover:no-underline">
                    <div class="w-full md:w-2/3 rounded-t">
                        <img src="/images/blogpost/{{$blogpost->images}}" class="h-full w-full shadow object-cover">
                    </div>

                    <div class="w-full md:w-1/3 flex flex-col flex-grow flex-shrink">
                        <div class="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
                            <p class="w-full text-gray-600 text-xs md:text-sm pt-6 px-6">GETTING STARTED</p>
                            <div class="w-full font-bold text-xl text-gray-900 px-6">{{$blogpost->title}}</div>
                        ...
                        <div class="flex-none mt-auto bg-white rounded-b rounded-t-none overflow-hidden shadow-lg p-6">
                            <div class="flex items-center justify-between">
                                    <img class="w-8 h-8 rounded-full mr-4 avatar" src="/images/profile/{{ $blogpost->user->profile_images }}"
                                    alt="Avatar of Author">
                                <p class="text-gray-600 text-xs md:text-sm">{{$blogpost->created_at->diffForHumans()}}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="flex flex-wrap justify-between pt-12 -mx-6">
                ...
                @foreach (App\Post::blogPost(1,3) as $blogpost)
                <!--1/3 col -->
                <div class="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink">
                    <div class="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
                        <a href="/view/{{$blogpost->id}}" class="flex flex-wrap no-underline hover:no-underline">
                            <img src="/images/blogpost/{{$blogpost->images}}"
                ...
                @foreach (App\Post::blogPost(4,2) as $blogpost)
                <!--1/2 col -->
                <div class="w-full md:w-1/2 p-6 flex flex-col flex-grow flex-shrink">
                    <div class="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
                        <a href="/view/{{$blogpost->id}}" class="flex flex-wrap no-underline hover:no-underline">
                            <img src="/images/blogpost/{{$blogpost->images}}"
                                class="h-full w-full rounded-t pb-6 object-cover">
                            <p class="w-full text-gray-600 text-xs md:text-sm px-6">GETTING STARTED</p>
                            <div class="w-full font-bold text-xl text-gray-900 px-6">{{$blogpost->title}}</div>
                            <p class="text-gray-800 font-serif text-base px-6 mb-5">
                                {{$blogpost->description}}
                ...
                @foreach (App\Post::blogPost(6,1) as $blogpost)
                <!--2/3 col -->
                ...
                @foreach (App\Post::blogPost(7,1) as $blogpost)
                <!--1/3 col -->
                <div class="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink">
                    <div class="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
                        <a href="/view/{{$blogpost->id}}" class="flex flex-wrap no-underline hover:no-underline">
